...
**Assignee profile must exist BEFORE dispatch.** The dispatcher skips tasks with `non-spawnable assignee — terminal lane, OK` when the profile doesn't exist. Before creating tasks, verify profiles exist with `hermes profile list`. If missing, create with `hermes profile create <name> --clone-from default`, then copy `.env`: `cp ~/.hermes/.env ~/.hermes/profiles/<name>/.env`. Then dispatch will pick it up.

**Hermes Workspace + Hermes Kanban (v2.3.0+).** Hermes Workspace (web UI, port 3333) now writes to the same `~/.hermes/kanban.db` as `hermes kanban` — tasks created in the Workspace UI ARE visible in `hermes kanban list` and vice versa. The mapping function `mapBoardStatus()` in `src/server/kanban-backend.ts` translates Workspace lanes to kanban statuses. If tasks created via Workspace UI never auto-dispatch, check for the `queued` bug (see `hermes-workspace-maintenance` skill). Valid dispatcher statuses are: `triage`, `todo`, `ready`, `running`, `blocked`, `done`, `archived` — anything else (like `queued`) is ignored by `dispatch_once()`.

**Dispatchers picks up ONLY `ready` status.** `dispatch_once()` in `kanban_db.py` queries `WHERE status = 'ready'`. Tasks in `todo` are auto-promoted to `ready` by `recompute_ready()` when all parent dependencies are satisfied. Tasks in any other status (`queued`, `review`, custom) are invisible to the dispatcher. If a task isn't being picked up, check: `hermes kanban show <id>` — if status isn't `ready`, fix it directly or via the source that created it.

**Subscribing notifications.** Use `hermes kanban notify-subscribe <task_id> --platform discord --chat-id <target>` to get gateway pings on status changes. Find available chat targets via `send_message(action='list')`.es profile list`. If missing, create with `hermes profile create <name> --clone-from default`, then copy `.env`: `cp ~/.hermes/.env ~/.hermes/profiles/<name>/.env`. Then dispatch will pick it up.

**Hermes Workspace + Hermes Kanban (v2.3.0+).** Hermes Workspace (web UI, port 3333) now writes to the same `~/.hermes/kanban.db` as `hermes kanban` — tasks created in the Workspace UI ARE visible in `hermes kanban list` and vice versa. The mapping function `mapBoardStatus()` in `src/server/kanban-backend.ts` translates Workspace lanes to kanban statuses. If tasks created via Workspace UI never auto-dispatch, check for the `queued` bug (see `hermes-workspace-maintenance` skill). Valid dispatcher statuses are: `triage`, `todo`, `ready`, `running`, `blocked`, `done`, `archived` — anything else (like `queued`) is ignored by `dispatch_once()`.

**Dispatchers picks up ONLY `ready` status.** `dispatch_once()` in `kanban_db.py` queries `WHERE status = 'ready'`. Tasks in `todo` are auto-promoted to `ready` by `recompute_ready()` when all parent dependencies are satisfied. Tasks in any other status (`queued`, `review`, custom) are invisible to the dispatcher. If a task isn't being picked up, check: `hermes kanban show <id>` — if status isn't `ready`, fix it directly or via the source that created it.

**Subscribing notifications.** Use `hermes kanban notify-subscribe <task_id> --platform discord --chat-id <target>` to get gateway pings on status changes. Find available chat targets via `send_message(action='list')`.
