...
**Hermes Workspace ≠ Hermes Kanban.** Hermes Workspace (web UI, port 3333) has its own internal Kanban board (Backlog → Todo → In Progress → Review) that is completely separate from the `hermes kanban` CLI system (`~/.hermes/kanban.db`). Workspace tasks are simple drag-drop cards with no dispatcher, profiles, or worktree isolation. The CLI kanban is the one with auto-dispatch, dependencies, and specialist routing. Don't confuse the two — `hermes kanban list` won't show Workspace tasks.
