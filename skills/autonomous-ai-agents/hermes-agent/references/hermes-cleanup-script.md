#!/bin/bash
# Hermes Agent weekly cleanup — runs every Sunday 4 AM
# Cleans temporary files and caches that are safe to remove
#
# USAGE:
#   1. Copy to scripts: cp this to ~/.hermes/scripts/hermes-cleanup.sh && chmod +x
#   2. Create cron job: no_agent=true, script=hermes-cleanup.sh, schedule="0 4 * * 0"
...
# 6. Hermes logs — keep current, delete rotated
